...
const HomeNav = () => {
    ...
    return (
        <nav className='p-4 border-b-2 w-full flex items-center justify-between'>
                ...
                <DropdownMenuContent>
                    ...
                    <DropdownMenuSeparator />
                    {
                        businessRoles.map((role, idx) => (
                            <DropdownMenuItem key={idx} onClick={() => setSelectedRole(role.name)}>{role.name}</DropdownMenuItem>
                        ))
                    }
                </DropdownMenuContent>
            </DropdownMenu>
            ...
            <div>
                <IoIosNotifications size={30} className='border border-accent rounded-full p-1 cursor-pointer' />
            </div>
        </nav>
    )
}
